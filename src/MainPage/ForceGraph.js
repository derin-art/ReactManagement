...
function ForceGraph({ datad, setRenderData, timesRan, Physics, setPhysics }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("resize", setDisplayWidth, setDisplayHeight);
    };
  }, []);
  ...
  return (
    <div className="w-screen h-screen">
      <div className="">
        ...
        >
          <div className="flex items-center justify-center p-1 absolute top-32">
            <div className="text-white font-Tilt font-bold text-2xl p-3 w-full">
              <div className="bg-black text-white p-2">ReactModels</div>
              <div className="text-xs bg-white text-black">
                <div>Drag on screen to move node</div>
                <div>Click on node to begin modelling</div>
              </div>
            </div>
          </div>
          <div className="flex justify-between p-1 pb-0 absolute z-50">
            <div className="flex items-start">
              <input
                className="border font-Tilt border-indigo-200 placeholder:font-Tilt mr-4 p-2"
                value={modelName}
                onChange={(e) => {
                  setIsSaved(false);
              ...
              <div className="flex flex-col">
                {" "}
                <button
                  className="font-Tilt border border-black p-2"
                  onClick={() => {
                    if (!modelName) return;
                    let localModels = JSON.parse(
                ...
                <button
                  className="font-Tilt text-xs p-2 border border-black mt-2"
                  onClick={() => {
                    setPhysics((prev) => {
                      if (prev === 1) {
                        return 0.9;
                ...
                >
                  Set {Physics === 1 ? "Physics" : "Static"} Mode
                </button>
              </div>
              <p className="font-Tilt p-2 font-bold text-indigo-300">
            ...
            <div className="">
              <div className="ml-32">
                <div
                  className="font-Tilt fixed z-50 pb-8 rounded-lg hover:shadow-lg"
                  onMouseOver={() => {
                    setPreviousMenuHover(true);
                  }}
                  onMouseLeave={() => {
                    setPreviousMenuHover(false);
                  }}
                >
                  {saveIcon}
                  {Object.keys(localModelsState).length > 0 &&
                    Object.keys(localModelsState).map((item) => {
                      return (
                        <div
                          className={`flex justify-between block animate-fadeIn mt-2 p-2 ${
                            !previousMenuHover && "hidden"
                          }`}
                          key={item}
                        >
                          <button
                            ...
                            className="text-sm"
                            onClick={() => {
                              const newLocalModels = JSON.parse(
                                localStorage.getItem("models")
                              );
                              setRenderData((prev) => {
                          ...
                          <button
                            ...
                            className="ml-8"
                            onClick={(e) => {
                              const newLocalModels = Object.entries(
                                localModelsState
                              ).filter(([key, val]) => {
                              ...
                              localStorage.setItem(
                                "models",
                                JSON.stringify(
                                  Object.fromEntries(newLocalModels)
                                )
                              );
                              setLocalModelsState(
                                JSON.parse(localStorage.getItem("models"))
          ...
          <div>
            {!openNode && (
              <div
                className={`absolute bg-gray-200 bg-opacity-75 p-8 font-Tilt w-fit text-white rounded-lg border-l-8 border-indigo-400 text-xs`}
                style={{
                  position: "absolute",
                  top: JSON.parse(localStorage.getItem("mode")).y - 50,
                  left: JSON.parse(localStorage.getItem("mode")).x,
                }}
              >
                <button
                  onClick={() => {
                  ...
                  className="bg-red-500 text-white p-1 text-sm rounded-lg mb-2"
                >
                  Close
                </button>

                <button
                  className="bg-gray-700 text-white p-2 text-sm rounded-lg mb-2 ml-2"
                  onClick={() => {
                ...
                <div className={`${createConnection && "hidden"} text-black`}>
                  <div className="text-xs font-Tilt text-green-400 font-bold">
                    Size and Name input required for render
                  </div>
                  <input
                    placeholder="new component name"
                    className="rounded-lg mb-2 p-1"
                  ...
                  <div className="flex">
                    <button
                      className={`p-1 bg-white mr-2 text-xs rounded-lg hover:bg-green-300 hover:text-white ${
                        newConnection.size === "1"
                    ...
                    <button
                      className={`p-1 bg-white text-xs rounded-lg hover:bg-green-300 hover:text-white ${
                        newConnection.size === "3"
                          ? "bg-green-400 text-white"
                          : ""
                      }`}
                      onClick={() => {
                  ...
                  <div className="mt-2 mb-2">
                    <button
                      className={`bg-red-500 p-2 text-white rounded-lg ${
                        newConnection.isProps && "bg-green-400"
                      }`}
                      onClick={() => {
                        setNewConnection((prev) => {
                    ...
                    >
                      {newConnection.isProps ? "Props" : "NoProps"}
                    </button>
                  </div>
                  ...
                  <div>
                    ...
                    ></textarea>
                    <textarea
                      placeholder="Link Relationship"
                      value={newConnection.linkDetails}
                      className="rounded-lg p-2 ml-2"
                      onChange={(e) => {
                        setNewConnection((prev) => ({
                          ...prev,
                          linkDetails: e.target.value,
                        }));
                      }}
                    ></textarea>
                  </div>
                  ...
                  <button
                    className={`mt-2 bg-gray-700 text-white rounded-lg p-2 mr-2 ${
                      currentNode.id === "a" && "hidden"
                    }`}
                    onClick={() => {
                      if (deleteCertainty) {
                        ...
                        const rendLinks = newLinks.map((link) => {
                          if (typeof (link.target == "object")) {
                            if (
                              ...
                              !foundIdArray.includes(link.target.id)
                            ) {
                              return link;
                            } else {
                              return;
                            ...
                            if (
                              ...
                              !foundIdArray.includes(link.target)
                            ) {
                              return link;
                            } else {
                              return;
                  ...
                  <button
                    className="mt-2 p-2 bg-gray-700 rounded-lg text-white"
                    onClick={() => {
                        ...
                        const DataTransformation = {
                          nodes: [
                            ...nodes,
                            {
                              ...
                              val: newConnection.size,
                              info: newConnection.info,
                              heirachy: NodeHierachy,
                              parents: currentNode.id,
                            },
                          ],
                          links: [
                            ...links,
                            {
                              ...
                              target: currentId,
                              isProps: newConnection.isProps,
                              recieveProps: false,
                              relDetails: newConnection.linkDetails,
                            },
                          ],
                        };
                        const ModifiedDataNode = DataTransformation.nodes.map(
                          (item) => {
                ...
                <div
                  className={`mt-2 font-bold ${
                    deleteCertainty ? "text-green-400" : "text-red-500"
                  } ${
                    !createConnection
                      ? deletePopup
                        ? "block"
                ...
                >
                  {deleteCertainty
                    ? `If you're sure click the delete button`
                    : `Are you sure you want to delete ${itemsToBeDeleted} item(s)?`}
                </div>
                <button
                  onClick={() => {
                    setDeleteCertainty(true);
                  }}
                  className={`text-white p-2 ${
                    !createConnection
                      ? deletePopup
                        ...
                        : "hidden"
                      : "hidden"
                  }  ${deleteCertainty ? "bg-green-400" : "bg-red-500"}`}
                >
                  GO ahead
                </button>
              </div>
