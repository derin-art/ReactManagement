...
class RichEditorExample extends React.Component {
  constructor(props) {
    ...
    if (JSON.parse(localStorage.getItem("noteColors"))) {
      noteLocalColor = JSON.parse(localStorage.getItem("noteColors"))[
        this.props.noteName
      ];
    } else {
      noteLocalColor = "blue";
    ...
    this.state = {
      editorState: isNoteAvailable
        ? EditorState.createWithContent(
            convertFromRaw(JSON.parse(content)[this.props.noteName])
          )
        : EditorState.createEmpty(),
      colorCode: noteLocalColor,
    };
  ...
  render() {
    ...
    const saveColor = (color) => {
      ...
      localStorage.setItem(
        "noteColors",
        JSON.stringify({ ...localColors, [this.props.noteName]: color })
      );
    };
    /*Rich max-w-[370px] w-[400px] max-h-[500px] */
    return (
      <div className="RichEditor-root overflow-x-hidden overflow-y-hidden p-2">
        <div className="relative">
          <div className="absolute -top-4 flex flex-col bg-white z-50 w-full p-1">
            <BlockStyleControls
            ...
            <div className="flex items-center justify-center text-xs font-Tilt">
              ...
              <button
                className="w-4 h-4 ml-2 bg-blue-500"
                onClick={() => {
              ...
              ></button>
              <button
                className="w-4 h-4 ml-2 bg-green-500"
                onClick={() => {
              ...
              ></button>
              <button
                className="w-4 h-4 ml-2 bg-yellow-500"
                onClick={() => {
                  this.setState({ colorCode: "yellow" });
                  saveColor("yellow");
                }}
              ></button>
              <button
                className="w-4 h-4 ml-2 bg-indigo-500"
                onClick={() => {
                  this.setState({ colorCode: "indigo" });
                  saveColor("indigo");
                }}
              ></button>
            </div>
        ...
        <div
          className={`${className} bg-${this.state.colorCode}-100 sm:max-h-[600px] max-h-[400px] overflow-x-hidden -z-10 pt-32 overflow-y-auto border-l-4 border-${this.state.colorCode}-500 p-2 `}
          onClick={this.focus}
        >
          <Editor
